...
## 4.2 Limitations of the IPC2002 correlation at large diameter

The benchmarking of Section 3 shows that the IPC2002 correlation, although appropriate for the 12–16 in pipes from which it was calibrated, should not be extrapolated to large-diameter, high-strength line pipe. Two failures appear. First, because the correlation enters geometry only through $D/t$, it cannot represent the residual diameter dependence observed at fixed $D/t = 73.1$, where the mean SCF rose by 13% from the 56 in to the 36 in pipe. Second, and more seriously, its $(a/C)^{-2.87}$ term inverts the physical trend: it lowers the predicted SCF as the ripple widens, whereas the finite element results—and the weakly positive fitted exponent of $+0.065$—show that a wider ripple is more severe. The consequence is a systematic, sign-changing error, from over-prediction by up to 84% for narrow ripples to under-prediction exceeding 400% for wide ones. The under-prediction is the dangerous direction, since it would classify a genuinely severe wide ripple as benign. The proposed correlation removes both errors within its stated envelope.
